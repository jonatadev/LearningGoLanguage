...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	for start.Before(nextDay) {
		...
		fmt.Fprintf(w, "%s,%s,%.2f,%d\n", start.
			Format(timeFmt), end.Format(timeFmt), distance,
			passengers)
	}
}

// The keyword chan is used to create channels.
// channel operator, <-, is used to send data to a channel.
// The channel operator, <-, is also used to receive data from a channel.
